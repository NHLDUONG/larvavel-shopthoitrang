...
class Cart //tạo 1 đối tượng cart
{
	...
	public function add($item, $id){// hàm thêm 1 sản phẩm
		...
		$giohang = ['qty'=>0, 'price' => $item->price_sale, 'item' => $item]; // qty=>tạo giỏ hàng gồm có  số lần mua sản phẩm , price=>giá của sản phẩm, item => là những thuộc tinhscuar sản phẩm đang mua (tên sản phẩm, giá sp, mã sp, .....) 
		if($item->size){ // lúc nảy bên PagesController nó có chỗ thêm cái size vào sản phẩm thỳ chỗ này là kiểm tra xem thử là có size không.
			$giohang['size'] = $item->size; //nếu sản phẩm mua có size thỳ gán size đó vào giỏ hàng.
		}
		if($item->color){ // lúc nảy bên PagesController nó có chỗ thêm cái color vào sản phẩm thỳ chỗ này là kiểm tra xem thử là có color không.
			$giohang['color'] = $item->color; //nếu sản phẩm mua có color thỳ gán color đó vào giỏ hàng.
		}
		//dd($giohang);
//die();
		...
		$giohang['price'] = $item->price_sale * $giohang['qty']; // tính giá sản phẩm bằng cách lấy số lượng sản phẩm đã mua nhân với đơn giá của sản phẩm.
	...
	public function update($item, $id, $sl){
		if($this->items){ // kiểm tra nếu như trong giỏ hàng đã có sản phẩm (tức là khi mua sản phẩm a , sau đó mua thêm sản phẩm a nữa thỳ sẽ vô if này)
			if(array_key_exists($id, $this->items)){
				$giohang = $this->items[$id]; // nếu như sản phẩm này đã dc mua rồi thỳ chỉ cần gán lại giỏ hàng.
			}
		}
		$giohang['qty'] = $sl; // tăng số lượng sản phẩm trong giỏ hàng lên 1.
		$giohang['price'] = $item->price_sale * $giohang['qty']; // tính giá sản phẩm bằng cách lấy số lượng sản phẩm đã mua nhân với đơn giá của sản phẩm.
		$this->items[$id] = $giohang; // gán lại id giỏ hàng.
		$this->totalQty = $this->totalQty + $sl; // tính tổng số lượng sản phẩm 
		$this->totalPrice += $item->price_sale; // tính tổng tiền của nhung sản phẩm đã mua
